...
#   - Load the basic QA data
#   - Mask NDVI with the QA data to leave only highest quality pixels
...
for(satellite in c("aqua", "terra")) {
  ...
  mclapply(date_dirs, function(date_dir) {
    ...
    qa <- lapply(tiles, function(tile) {
      ...
      .[grepl("1 km monthly pixel reliability$", .)] %>%
      raster
    }) %>% do.call(merge, .) %>% crop(., grid, snap = "out")
    # 5 seconds
    # 4.52 MB

    # Mask NDVI with the QA data keeping only the highest-quality pixels
    # TODO look at using lower-quality pixels as well
    ndvi[qa != 0] <- NA

    # Create a table to hold the extracted data
    result <- data.frame("modis_grid_id" = grid$id, "date" = basename(date_dir))
